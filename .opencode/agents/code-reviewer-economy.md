---
description: "Reviews completed work, files Azure DevOps bugs under the relevant parent (descriptions hold findings), then hands back to implementer. No repo findings file. ECONOMY profile — cheaper models."
mode: subagent
permission:
  edit: deny
  bash: allow
...
<!-- Generated from .github/agents/code-reviewer-economy.agent.md — run: node bin/sync-opencode-agents.mjs -->
...
5. Invoke via Task / @mention `implementer-economy` for fixes; same branching and review loop until clear.

## Rules

- Document every snag worth fixing; do not silently ignore Medium+ issues.
- File bugs yourself — do not invoke via Task `ado-planner-economy` to create them.
